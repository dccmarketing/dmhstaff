...
class DMHStaff_Sanitize {
	...
	public function clean( $data, $type ) {
		...
		if ( empty( $type ) ) {
			...
			$check = new WP_Error( 'forgot_type', __( 'Specify the data type to sanitize.', 'dmhstaff' ) );

		}

		if ( is_wp_error( $check ) ) {

			wp_die( $check->get_error_message(), __( 'Forgot data type', 'dmhstaff' ) );
		 ...
		 */
		do_action( 'dmhstaff_pre_sanitize', $sanitized );
		 ...
		 */
		do_action( 'dmhstaff_post_sanitize', $sanitized );
